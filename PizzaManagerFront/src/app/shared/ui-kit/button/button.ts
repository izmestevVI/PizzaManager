...
export class ButtonComponent {
  ...
  hostClasses = computed(() => {
    const base =
      'inline-flex justify-center items-center gap-2 font-semibold transition-all duration-200 rounded-xl whitespace-nowrap shadow-sm active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none';
    ...
    const variants = {
      ...
      outline:
        'bg-transparent border border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-900 text-slate-700 dark:text-slate-300',
    ...
    return [base, variants[this.variant()], sizes[this.size()], this.fullWidth() ? 'w-full' : 'w-auto'].join(' ');
  });
}
